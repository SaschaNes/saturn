...
IPMI STONITH fences a failed primary. DRBD protocol C and majority quorum protect
the data. **Do not run both modes, do not run the old `rsync` replication, and
do not let systemd start Samba independently.** This profile assumes three
...
BMC passwords. No real cluster was available to validate the resulting plan.
Neither the template nor the tests certify a production installation.
...
1. Ubuntu 24.04 or 26.04, Pacemaker, Corosync, `pcs`, `resource-agents-extra`,
   ...
   DRBD version in place of DRBD 9.
2. Three independent backing devices, a dedicated replication network,
   ...
   privileges. Verify `fence_ipmilan` and `passwd_script` support with
   `pcs stonith describe fence_ipmilan` on the installed versions. If your
   BMC supports **Redfish only**, this IPMI profile is not applicable: select
   and test a Redfish fencing agent before enabling automatic failover.
...
the legacy service state and fencing-secret file permissions. It cannot test
that fencing really powers off a node or that DRBD data is synchronized.
...
run `python3 ha/status.py /root/saturn-cluster.json` from a cluster node.
...
VIP. Its loopback-only `/status`, `/health`, `/primary` and `/standby` endpoints
return 503 for unhealthy cluster state; `/standby` does not prove that DRBD
data is UpToDate. Do not expose these endpoints as a failover authority.
